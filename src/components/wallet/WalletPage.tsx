...
import { Copy, ExternalLink, RefreshCw, CheckCircle2, AlertCircle, Shield, BookOpen } from 'lucide-react';
...
const WALLETS = [
  ...
  { key: 'rewards',   label: 'Rewards Wallet',     address: 'GDVTACGR5QEVND7LW56QLQAXYBWXVAZOBRUCZ3Z2ACRKKZL3VUHG4VYX',  color: 'from-green-500 to-emerald-500', icon: '🎁' },
...
async function fetchBal(address: string) {
  if (!address?.startsWith('G')) return { xlm: '—', zenith: '—' };
  try {
    const res  = await fetch(`${HORIZON}/accounts/${address}`);
    if (!res.ok) return { xlm: '0', zenith: '0' };
    const data = await res.json();
    let xlm = '0', zenith = '0';
    for (const b of data.balances) {
      if (b.asset_type === 'native')
        xlm = parseFloat(b.balance).toLocaleString('en', { maximumFractionDigits: 2 });
      if (b.asset_code === 'ZENITH' && b.asset_issuer === ZENITH_ISSUER)
        zenith = parseFloat(b.balance).toFixed(2);
    }
    return { xlm, zenith };
  } catch { return { xlm: '—', zenith: '—' }; }
}

export function WalletPage() {
  const [bals, setBals]     = useState<Record<string, { xlm: string; zenith: string; loading: boolean }>>({});
  const [copied, setCopied] = useState<string | null>(null);

  const load = async (key: string, address: string) => {
    setBals(p => ({ ...p, [key]: { xlm: '—', zenith: '—', loading: true } }));
  ...
  return (
    <div className="space-y-4 p-4 pb-24">
      ...
      <button onClick={() => WALLETS.forEach(w => load(w.key, w.address))}
        className="flex items-center gap-2 text-xs text-gray-400 hover:text-white px-3 py-1.5 bg-gray-800 rounded-lg ml-auto">
        <RefreshCw className="w-3.5 h-3.5" /> تحديث الأرصدة
      </button>

      {WALLETS.map(w => {
        const b = bals[w.key] || { xlm: '—', zenith: '—', loading: false };
        return (
